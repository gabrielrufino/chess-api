...
describe('GameDto Decorators', () => {
  it('should evaluate type functions for swagger properties without calling them', () => {
    const metaWhite = Reflect.getMetadata('swagger/apiModelProperties', GameDto.prototype, 'whitePlayer');
    expect(metaWhite?.type).toBeDefined();

    const metaBlack = Reflect.getMetadata('swagger/apiModelProperties', GameDto.prototype, 'blackPlayer');
    expect(metaBlack?.type).toBeDefined();
    
    const metaData = Reflect.getMetadata('swagger/apiModelProperties', GameListDto.prototype, 'data');
    expect(metaData?.type).toBeDefined();
  });

  it('should transform to correct classes', () => {
    const gameList = plainToInstance(GameListDto, { data: [{ whitePlayer: {}, blackPlayer: {} }] });
